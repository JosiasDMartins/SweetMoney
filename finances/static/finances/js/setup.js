...
// ============================================
// RELOAD PAGE BUTTON (After Migration)
// ============================================

function initReloadPageButton() {
    const reloadButtons = document.querySelectorAll('[data-action="reload-page"]');

    if (!reloadButtons.length) {
        console.log('[SETUP] No reload buttons found (expected when not in migration state)');
        return;
    }

    reloadButtons.forEach(button => {
        button.addEventListener('click', function() {
            console.log('[SETUP] Reloading page after migration...');
            window.location.reload();
        });
    });

    console.log('[SETUP] Reload page buttons initialized');
}
